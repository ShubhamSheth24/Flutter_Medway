...
class _MedicineScreenState extends State<MedicineScreen> {
  ...
  void _addToCart() {
    Provider.of<Cart>(context, listen: false).addToCart({
      'id': widget.product["id"] ?? UniqueKey().toString(), // Ensure unique ID
      ...
      'quantity': quantity, // Use local quantity for cart
  ...
  Widget build(BuildContext context) {
    final double price = double.tryParse(widget.product["price"].toString().replaceAll('₹', '')) ?? 0.0;
        ...
        actions: [
          IconButton(
            icon: Stack(
              ...
              children: [
                const Icon(Icons.shopping_cart, color: Colors.black87, size: 28),
                Positioned(
                  right: 0,
                  top: 0,
                  child: Consumer<Cart>(
            ...
            children: [
              ...
              Row(
                children: List.generate(5, (index) => Icon(
                  index < (widget.product["rating"]?.toInt() ?? 0) ? Icons.star : Icons.star_border,
                  color: Colors.amber,
                  size: 22,
                )),
              ),
                  ...
                  Row(
                    children: [
                      ...
                      _buildIconButton(
                        ...
                        onTap: () {
                          setState(() => quantity++);
                          _addToCart(); // Add to cart on plus icon press
                        },
                      ),
                    ],
                  ),
  ...
  Widget _buildIconButton({required IconData icon, Color color = Colors.grey, VoidCallback? onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, color: onTap != null ? color : Colors.grey[400], size: 24),
      ),
    );
  }
}