...
class _MedicineReminderState extends State<MedicineReminder> {
  ...
  Future<void> _scheduleNotification(Reminder reminder) async {
    for (int i = 0; i < reminder.times.length; i++) {
      ...
      try {
        ...
        debugPrint(
            'Scheduled notification for ${reminder.medicine} at $scheduledDate');
      } catch (e) {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(
            labelText:
                'Caregiver Phone Number (e.g., +919876543210 or 9876543210)',
            labelStyle: TextStyle(fontSize: 18),
            border: OutlineInputBorder(),
          ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                // Ensure proper URI encoding and format
                final Uri smsUri = Uri(
                    scheme: 'sms',
                    path: phoneNumber,
                    queryParameters: {'body': message});
                debugPrint('Attempting to launch: $smsUri'); // Debug the URI
                if (await canLaunchUrl(smsUri)) {
                  await launchUrl(smsUri);
                  debugPrint('Opened SMS app for $phoneNumber: $message');
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text('SMS app opened - please press send')));
                } else {
                  debugPrint('Could not launch SMS app for URI: $smsUri');
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                      content: Text(
                          'No SMS app found - please set a default SMS app')));
                }
              } catch (e) {
            ...
            child: const Text('Open SMS', style: TextStyle(fontSize: 18)),
            style: ElevatedButton.styleFrom(
                padding:
                    const EdgeInsets.symmetric(vertical: 15, horizontal: 20)),
          ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        padding: defaultPadding().copyWith(top: 40),
        child: Column(
